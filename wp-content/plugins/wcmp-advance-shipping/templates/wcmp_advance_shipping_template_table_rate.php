...
<script type="text/javascript">
    jQuery(document).ready(function ($) {
        $('.wcmp_add_tablerate_item').on('click', function () {
            ...
            item.find('input, select').each(function (index, element) {
                ...
                var name = 'wcmp_table_rate[' + elm.data('instance_id') +'][' + itemCount + '][' + elm.data('name') + ']';
                elm.attr('name', name);
            });
            $(this).closest('table').find('tbody').append(item);
        });

        $('.wcmp_remove_table_rate_item').on('click', function () {
            var checkedRate = [];
            $('.wcmp_table_rate_shipping input[type=checkbox]').each(function () {
